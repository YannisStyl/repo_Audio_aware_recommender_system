...
from Training_Scripts.t2b_fx_and_voice_GRPO import DualStreamConditionedQwen
...
def resolve_checkpoint_dir(checkpoint_path: str) -> str:
    ...
    if os.path.isdir(checkpoint_path):
        ...
        if subdirs:
            ...
            print(f"[FX+Voice Inference] Auto-detected latest checkpoint: {latest}")
            return latest
    return checkpoint_path


def load_dual_model_and_tokenizer(checkpoint_dir: str, base_model_name: str = "Qwen/Qwen3.5-0.8B", device: str = "cuda"):
    """
    Loads base LLM, applies LoRA structure, injects FX and Voice Q-Former adapters,
    ...
    print(f"[FX+Voice Inference] Loading tokenizer...")
    try:
        tokenizer = AutoTokenizer.from_pretrained(resolved_ckpt, trust_remote_code=True)
    except Exception:
        tokenizer = AutoTokenizer.from_pretrained(base_model_name, trust_remote_code=True)

    special_tokens = ["<|audio|>", "<|voice|>"]
    missing_tokens = [t for t in special_tokens if t not in tokenizer.get_vocab()]
    if missing_tokens:
    ...
    print(f"[FX+Voice Inference] Loading base LLM: {base_model_name}...")
    ...
    print(f"[FX+Voice Inference] Loading weights from safetensors: {st_file}")
    ...
    peft_sd = {
        ...
        for k, v in sd.items() if not (k.startswith("fx_projector.") or k.startswith("voice_projector."))
    }

    res_peft = model.qwen.load_state_dict(peft_sd, strict=False)
    res_fx = model.fx_adapter.load_state_dict(fx_sd, strict=False)
    res_voice = model.voice_adapter.load_state_dict(voice_sd, strict=False)
    
    print(f"[FX+Voice Inference] Model loaded successfully! (PEFT unmapped: {len(res_peft.unexpected_keys)}, FX Projector unmapped: {len(res_fx.unexpected_keys)}, Voice Projector unmapped: {len(res_voice.unexpected_keys)})")
    model.eval()
...
def run_inference(
    ...
    """
    Executes inference for a text instruction + FX feature tensor + Voice feature tensor.
    """
    messages = [
    ...
    # Attach FX and Voice tensors into model caches using prompt text as key
    clean_prompt = prompt.strip()
    model.prompt_to_fx[clean_prompt] = fx_tensor
...
def main():
    parser = argparse.ArgumentParser(description="Inference script for Dual-Stream (FX + Voice) GRPO model")
    parser.add_argument("--checkpoint", type=str, default=os.path.join(script_dir, "FX_Voice_model"), help="Path to FX+Voice model directory or checkpoint")
    parser.add_argument("--fx_features_path", type=str, default=os.path.join(src_dir, "..", "Data_dir", "prompt_to_fx_windowed.pkl"), help="Path to pre-extracted FX feature pickle file")
    ...
    parser.add_argument("--prompt", type=str, default=None, help="Text instruction prompt (e.g. 'Enhance vocal clarity and boost bass depth')")
    ...
    model, tokenizer = load_dual_model_and_tokenizer(args.checkpoint, device=args.device)

    # Load FX feature pickle
    fx_dict = {}
    fx_paths_to_try = [
        args.fx_features_path,
        os.path.join(src_dir, "..", "Listening_Exp", "Perceptual_evaluation", "perceptual_prompt_to_fx.pkl"),
        os.path.join(src_dir, "..", "Data_dir", "perceptual_prompt_to_fx.pkl"),
    ]
    for p in fx_paths_to_try:
        if p and os.path.exists(p):
            print(f"[FX+Voice Inference] Loading FX feature dictionary from: {p}")
            with open(p, "rb") as f:
                raw_fx = pickle.load(f)
            fx_dict.update({k.strip(): (v["tensor"] if isinstance(v, dict) and "tensor" in v else v) for k, v in raw_fx.items()})

    # Load Voice feature pickle
    voice_dict = {}
    voice_paths_to_try = [
    ...
    for p in voice_paths_to_try:
        if p and os.path.exists(p):
            print(f"[FX+Voice Inference] Loading Voice feature dictionary from: {p}")
            with open(p, "rb") as f:
                raw_voice = pickle.load(f)
            voice_dict.update({k.strip(): (v["tensor"] if isinstance(v, dict) and "tensor" in v else v) for k, v in raw_voice.items()})


    if args.prompt:
        prompts_to_process = [args.prompt]
    else:
        # Pick sample prompts that exist in both feature dicts
        common_keys = list(set(fx_dict.keys()) & set(voice_dict.keys()))
        if common_keys:
            prompts_to_process = common_keys[:3]
            print(f"[FX+Voice Inference] No --prompt supplied. Running sample prompts from dataset: {prompts_to_process}")
        else:
            prompts_to_process = ["Enhance the vocal clarity and boost low bass"]

    for prompt in prompts_to_process:
        clean_p = prompt.strip()
        fx_tensor = fx_dict.get(clean_p, torch.zeros(10, 2048, dtype=torch.bfloat16))
        voice_tensor = voice_dict.get(clean_p, torch.zeros(10, 768, dtype=torch.bfloat16))

        if clean_p not in fx_dict:
            print(f"[FX+Voice Inference] Prompt not in FX feature dict. Using dummy zero feature sequence [10, 2048].")
        if clean_p not in voice_dict:
            print(f"[FX+Voice Inference] Prompt not in Voice feature dict. Using dummy zero feature sequence [10, 768].")

        completions, parsed = run_inference(
            model, tokenizer, clean_p, fx_tensor, voice_tensor,
