...
from Training_Scripts.t2b_fx_GRPO import AudioConditionedQwen
...
def resolve_checkpoint_dir(checkpoint_path: str) -> str:
    ...
    if os.path.isdir(checkpoint_path):
        ...
        if subdirs:
            ...
            print(f"[FX Inference] Auto-detected latest checkpoint: {latest}")
            return latest
    return checkpoint_path


def load_fx_model_and_tokenizer(checkpoint_dir: str, base_model_name: str = "Qwen/Qwen3.5-0.8B", device: str = "cuda"):
    """
    Loads base LLM, applies LoRA structure, injects FX Q-Former adapter,
    ...
    print(f"[FX Inference] Loading tokenizer...")
    try:
    ...
    print(f"[FX Inference] Loading base LLM: {base_model_name}...")
    ...
    print(f"[FX Inference] Loading weights from safetensors: {st_file}")
    ...
    peft_sd = {
        ...
        for k, v in sd.items() if not k.startswith("fx_projector.")
    }

    res_peft = model.qwen.load_state_dict(peft_sd, strict=False)
    res_fx = model.fx_adapter.load_state_dict(fx_sd, strict=False)
    
    print(f"[FX Inference] Model loaded successfully! (PEFT unmapped: {len(res_peft.unexpected_keys)}, Projector unmapped: {len(res_fx.unexpected_keys)})")
    model.eval()
...
def run_inference(
    ...
    """
    Executes inference for a text instruction + FX feature tensor.
    """
    messages = [
    ...
    # Attach FX tensor into model's prompt_to_fx cache using prompt text as key
...
def main():
    parser = argparse.ArgumentParser(description="Inference script for FX-conditioned GRPO model")
    parser.add_argument("--checkpoint", type=str, default=os.path.join(script_dir, "FX_model"), help="Path to FX model directory or checkpoint")
    ...
    parser.add_argument("--prompt", type=str, default=None, help="Text instruction prompt (e.g. 'Boost the low bass and make it warm')")
    ...
    model, tokenizer = load_fx_model_and_tokenizer(args.checkpoint, device=args.device)
    ...
    for p in feature_paths_to_try:
        if p and os.path.exists(p):
            print(f"[FX Inference] Loading feature dictionary from: {p}")
            with open(p, "rb") as f:
                raw_data = pickle.load(f)
            loaded_dict = {k.strip(): (v["tensor"] if isinstance(v, dict) and "tensor" in v else v) for k, v in raw_data.items()}
            fx_dict.update(loaded_dict)


    if args.prompt:
        prompts_to_process = [args.prompt]
    else:
        # If no prompt specified, pick a few sample prompts from the feature dict or prompt interaction loop
        if fx_dict:
            prompts_to_process = list(fx_dict.keys())[:3]
            print(f"[FX Inference] No --prompt supplied. Running sample prompts from dataset: {prompts_to_process}")
        else:
            prompts_to_process = ["Enhance the bass kick and add depth to the mix"]

    for prompt in prompts_to_process:
        clean_p = prompt.strip()
        if clean_p in fx_dict:
            fx_tensor = fx_dict[clean_p]
        else:
            print(f"[FX Inference] Prompt not in feature dict. Using dummy zero feature sequence [10, 2048].")
            fx_tensor = torch.zeros(10, 2048, dtype=torch.bfloat16)

        completions, parsed = run_inference(
            model, tokenizer, clean_p, fx_tensor,
