...
from Training_Scripts.prompts import SYSTEM_PROMPT_AUDIO, SYSTEM_PROMPT_NO_AUDIO
...
class QFormerAdapter(nn.Module):
    """
    Q-Former cross-attention adapter for the FX stream.
    Projects chronological Fx-Encoder++ conv6 features [T, 2048] into
    Qwen's embedding space via 16 learnable query tokens and multi-head
    cross-attention, emitting 16 fixed-length audio embeddings per sample.

    The enc_dim parameter controls the input projection; everything else
    (num_queries, num_heads, llm_dim) matches the dual-stream version so
    checkpoints are structurally compatible.
    """
...
class TextMaskSchedule:
    ...
    pressure on the Q-Former to extract a signal from audio features alone,
...
# =====================================================================
# 3. THE FX-ONLY MULTIMODAL WRAPPER (GRPO COMPATIBLE)
# =====================================================================
class AudioConditionedQwen(nn.Module):
    """
    Single-stream FX-only audio-conditioned wrapper around a PEFT-wrapped
    Qwen model.

    One QFormerAdapter bridges Fx-Encoder++ conv6 features into Qwen's
    embedding space:
    ...
    16 audio tokens are injected per prompt. The encoder is not loaded
    during training — features are pre-extracted and cached in prompt_to_fx.

    NOTE ON JOIN KEYS: prompt_to_fx is keyed by prompt text (see
    _recover_features). fx_ids is an optional companion dict
    ...
    def __init__(
        ...
        if use_audio and (tokenizer is None or fx_token_id is None):
            raise ValueError(
                "AudioConditionedQwen(use_audio=True) requires "
                "`tokenizer` and `fx_token_id` to be passed explicitly."
            )
    ...
    def _recover_features(self, input_ids):
        """
        Decode input_ids, match each sequence to its pre-extracted FX
        ...
        on either is trustworthy. A prior version of this method included a
        substring-scan fallback — that has been removed: it could silently
        match the wrong prompt whenever one prompt's text is a substring of
        another's (a real risk given how similar paraphrases are within this
        dataset). Failing loudly via the RuntimeError below is preferable to
    ...
    def _get_user_turn_mask(self, token_ids_1d: torch.Tensor) -> torch.Tensor:
        ...
        closing <|im_end|>), with audio placeholder positions forced back
    ...
    # ------------------------------------------------------------------
    def get_inputs_embeds(self, input_ids, fx_features=None, fx_mask=None):
        if fx_features is None or fx_mask is None:
            fx_features, fx_mask = self._recover_features(input_ids)
    ...
    def forward(self, input_ids, attention_mask=None, **kwargs):
        # Pop FX tensors if a caller supplies them directly,
        ...
        inputs_embeds = self.get_inputs_embeds(input_ids, fx_features, fx_mask)
        return self.qwen(
    ...
    def generate(self, input_ids=None, **kwargs):
        ...
        for key in ("fx_features", "fx_mask"):
            kwargs.pop(key, None)
...
# 4b. CALLBACKS
# =====================================================================
class AudioConditioningCallback(TrainerCallback):
    """
    Logs gradient norms and weight norms for the fx_adapter to wandb, and
    ...
    def on_pre_optimizer_step(self, args, state, control, **kwargs):
        ...
        if model is None:
            return
        adapter = getattr(model, "fx_adapter", None)
        if adapter is None:
            return
        for name, param in adapter.named_parameters():
            if param.requires_grad:
                self._grad_norms[f"adapter_grad_norm/fx_adapter/{name}"] = (
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        ...
        if adapter is not None:
            for name, param in adapter.named_parameters():
                if param.requires_grad:
                    logs[f"adapter_weight_norm/fx_adapter/{name}"] = param.norm().item()
...
class AdapterWarmupCallback(TrainerCallback):
    ...
    updates so the randomly-initialised fx_adapter gets a chance to learn a
    real signal before the LLM's LoRA weights start adapting around it
    (BLIP-2-style staged training).

    The fx_adapter trains from step 0 regardless of this callback — only
...
if __name__ == "__main__":
    ...
    ADAPTER               = "Fx_Qformer" if USE_AUDIO_CONDITIONING else "No_adapter"
    ...
    RUN_NAME = f"FX-{DIVERSITY_SCHEDULE}-beta_{GRPO_BETA_VALUE}-numgen_{NUM_GEN}"
    ...
    print(f"Token IDs — <|audio|>: {FX_TOKEN_ID}")
    ...
    # prompt_to_fx_windowed.pkl maps prompt text -> a dict with keys
    # "tensor" (the actual feature tensor, used for training) and
    # "sample_id"/"track"/"window_index"/"window_start_sec" (provenance
    # metadata — see fx_sample_id_provenance.pkl for the full reverse-lookup
    ...
    # ---- Instantiate FX-only wrapper ----
    model = AudioConditionedQwen(
        base_model,
    ...
    system_prompt = SYSTEM_PROMPT_AUDIO if USE_AUDIO_CONDITIONING else SYSTEM_PROMPT_NO_AUDIO
    ...
    # ---- GRPOConfig ----
    print(f"\nStarting FX-Only Q-Former GRPO Training\n")
    config = GRPOConfig(
        bf16=True,
