...
from Training_Scripts.prompts import SYSTEM_PROMPT_VOICE, SYSTEM_PROMPT_NO_AUDIO
...
class QFormerAdapter(nn.Module):
    """
    Q-Former cross-attention adapter for the Voice stream.
    Projects chronological Auden-Voice Zipformer features [T, 768] into
    Qwen's embedding space via 16 learnable query tokens and multi-head
    cross-attention, emitting 16 fixed-length voice embeddings per sample.

    The enc_dim parameter controls the input projection (768 for voice);
    everything else (num_queries, num_heads, llm_dim) matches the dual-stream
    version so checkpoints are structurally compatible.
    """
    def __init__(self, enc_dim=768, llm_dim=1024, num_queries=16, num_heads=8, dropout=0.1):
        super().__init__()
...
class TextMaskSchedule:
    ...
    pressure on the Q-Former to extract a signal from voice features alone,
    breaking the text-shortcut that causes train/eval generalisation gaps
    when each training prompt maps to exactly one audio file.
...
# =====================================================================
# 3. THE VOICE-ONLY MULTIMODAL WRAPPER (GRPO COMPATIBLE)
# =====================================================================
class VoiceConditionedQwen(nn.Module):
    """
    Single-stream Voice-only audio-conditioned wrapper around a PEFT-wrapped
    Qwen model.

    One QFormerAdapter bridges Auden-Voice Zipformer features into Qwen's
    embedding space:

      Voice stream (paralinguistic):
        Auden-Voice Zipformer features [T_voice, 768] → voice_adapter → 16 embeddings
        Injected at <|voice|> placeholder tokens (VOICE_TOKEN_ID)

    16 voice embeddings are injected per prompt. The encoder is not loaded
    during training — features are pre-extracted and cached in prompt_to_voice.

    NOTE ON JOIN KEYS: prompt_to_voice is keyed by prompt text (see
    _recover_features). voice_ids is an optional companion dict
    (prompt text -> stable sample_id, e.g. "TrackName.wav::w3") carried
    through purely for provenance/debugging and for external tooling
    (causal audio-swap grounding test, perceptual-study stimulus generation)
    — it is not consulted during the forward pass itself.
    """
    def __init__(
        self,
        qwen_model,
        ...
        # Provenance-only companion dict — not used for lookup during training.
        self.voice_ids = voice_ids if voice_ids is not None else {}

        self.text_mask_schedule = text_mask_schedule

        if use_audio and (tokenizer is None or voice_token_id is None):
            raise ValueError(
                "VoiceConditionedQwen(use_audio=True) requires "
                "`tokenizer` and `voice_token_id` to be passed explicitly."
            )
        ...
        # Voice Q-Former: Auden-Voice Zipformer output dim = 768
    ...
    def _recover_features(self, input_ids):
        """
        Decode input_ids, match each sequence to its pre-extracted Voice
        feature tensor, then batch-pad and generate the key padding mask.
        ...
        on either is trustworthy. Failing loudly via the RuntimeError below
        is preferable to a silent wrong match.
        ...
        voice_mask   = torch.ones(B, max_T_voice, dtype=torch.bool, device=dev)  # True = ignored
        for i, feat in enumerate(batch_voice):
            T = feat.size(0)
            padded_voice[i, :T, :] = feat.to(dev, dtype=torch.bfloat16)
            voice_mask[i,   :T]    = False  # Unmask valid frames

        return padded_voice, voice_mask
    ...
    def _get_user_turn_mask(self, token_ids_1d: torch.Tensor) -> torch.Tensor:
        ...
        closing <|im_end|>), with voice placeholder positions forced back
        to False so they are never zeroed out.
        ...
        # Voice placeholder tokens must NOT be zeroed — their embeddings are
        # overwritten by the adapter immediately after this masking step.
    ...
    # ------------------------------------------------------------------
    def get_inputs_embeds(self, input_ids, voice_features=None, voice_mask=None):
        if voice_features is None or voice_mask is None:
            voice_features, voice_mask = self._recover_features(input_ids)
    ...
    def forward(self, input_ids, attention_mask=None, **kwargs):
        # Pop Voice tensors if a caller supplies them directly,
        ...
        inputs_embeds = self.get_inputs_embeds(input_ids, voice_features, voice_mask)
        return self.qwen(
    ...
    def generate(self, input_ids=None, **kwargs):
        ...
        for key in ("voice_features", "voice_mask"):
            kwargs.pop(key, None)
...
def make_grpo_prediction_reward(all_reward_models, val_prompts=None):
    """
    Factory that closes over `all_reward_models`. Reward computation is
    purely text-based and stream-agnostic.
    """
...
def make_grpo_diversity_reward(schedule_state: ScheduleState, uniqueness_based: bool = False):
    ...
    Completions are text strings — stream-agnostic.
    """
...
# 4b. CALLBACKS
# =====================================================================
class VoiceConditioningCallback(TrainerCallback):
    """
    Logs gradient norms and weight norms for the voice_adapter to wandb, and
    keeps both ScheduleState objects in sync with the trainer's global_step.
    """
    def __init__(
        self,
    ...
    def on_pre_optimizer_step(self, args, state, control, **kwargs):
        ...
        if model is None:
            return
        adapter = getattr(model, "voice_adapter", None)
        if adapter is None:
            return
        for name, param in adapter.named_parameters():
            if param.requires_grad:
                self._grad_norms[f"adapter_grad_norm/voice_adapter/{name}"] = (
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        ...
        if adapter is not None:
            for name, param in adapter.named_parameters():
                if param.requires_grad:
                    logs[f"adapter_weight_norm/voice_adapter/{name}"] = param.norm().item()
...
class AdapterWarmupCallback(TrainerCallback):
    ...
    updates so the randomly-initialised voice_adapter gets a chance to learn a
    real signal before the LLM's LoRA weights start adapting around it
    (BLIP-2-style staged training).

    The voice_adapter trains from step 0 regardless of this callback — only
...
class CustomGRPOTrainer(GRPOTrainer):
    ...
    Phase 1: The Q-Former adapter trains; LoRA is frozen.
...
if __name__ == "__main__":
    ...
    ADAPTER               = "Voice_Qformer" if USE_AUDIO_CONDITIONING else "No_adapter"
    ...
    RUN_NAME = f"Voice-{DIVERSITY_SCHEDULE}-beta_{GRPO_BETA_VALUE}-numgen_{NUM_GEN}"
    ...
    special_tokens_dict = {'additional_special_tokens': ['<|voice|>']}
    tokenizer.add_special_tokens(special_tokens_dict)
    base_model.resize_token_embeddings(len(tokenizer))

    VOICE_TOKEN_ID = tokenizer.convert_tokens_to_ids('<|voice|>')
    print(f"Token IDs — <|voice|>: {VOICE_TOKEN_ID}")
    ...
    # prompt_to_voice_windowed.pkl maps prompt text -> a dict with keys
    # "tensor" (the actual feature tensor, used for training) and
    # "sample_id"/"track"/"window_index"/"window_start_sec" (provenance
    # metadata — see voice_sample_id_provenance.pkl for the full reverse-lookup
    # table, useful for the causal audio-swap test).
    ...
    # ---- Instantiate Voice-only wrapper ----
    ...
    system_prompt = SYSTEM_PROMPT_VOICE if USE_AUDIO_CONDITIONING else SYSTEM_PROMPT_NO_AUDIO
    ...
    # ---- GRPOConfig ----
    print(f"\nStarting Voice-Only Q-Former GRPO Training\n")
    config = GRPOConfig(
        bf16=True,
